#
# R code for exercise 3 (a) of homework 6 (series: U.S. industrial production)
#

# choose max lag
max.lag <- 24
# maximum lag for p-values for Ljung-Box statistics
example.str <- "exercise 3(a)"
...
ts.data <- ip
ts.desc <- "U.S. industrial production"
...
# fit SARIMA model
arima.fit <- arima(x = ts.data.raw, order = c(0, 1, 0), seasonal = list(
   order = c(0, 0, 0),
   period = 12
...
theo.acf <- ARMAacf(ar = ar.coef, ma = ma.coef, lag.max = max.lag, pacf = FALSE)
theo.pacf <- c(1, ARMAacf(ar = ar.coef, ma = ma.coef, lag.max = max.lag, pacf = TRUE))
# plot time series and associated ac.f. and pac.f.
layout(matrix(c(1, 1, 2, 3, 4, 4, 5, 6), nr = 2, byrow = T))
par(oma = c(0, 2, 12, 2))
plot(
   x = ts.data.raw, type = "b", xlab = "time", ylab = "", main = "raw series",
...
plot(
   x = ts.data.1stdiff, type = "b", xlab = "time", ylab = "",
   main = "first differences of raw series", col = "green4", font.main = 1, cex.main = 1
)
ts.acf <- acf(x = ts.data.1stdiff, type = "correlation", plot = FALSE)
plot(ts.acf,
   ylim = c(min(c(ts.acf$acf, theo.acf[-1])), max(c(ts.acf$acf, theo.acf[-1]))),
   xlab = expression(paste("lag ", tau, sep = "")),
   ylab = expression(paste("ac.f.(", tau, ")", sep = "")), main = ""
)
points(x = 0:(length(theo.acf) - 1), y = theo.acf, type = "p", col = "red")
title(main = "sample ac.f.", font.main = 1, cex.main = 1)
ts.pacf <- acf(x = ts.data.1stdiff, type = "partial", plot = FALSE)
plot(ts.pacf,
   ylim = c(
...
points(x = 0:(length(theo.pacf) - 1), y = theo.pacf, type = "p", col = "red")
title(main = "sample pac.f.", font.main = 1, cex.main = 1)
mtext(
   text = paste("Exercise 6.3 (a): ", ts.desc, " (raw series)", sep = ""),
...
plot(
   ...
   main = "standardized residuals", col = "green4", font.main = 1, cex.main = 1
)
acf(
   x = arima.fit$residuals, lag.max = max.lag, type = "correlation",
   xlab = expression(paste("lag ", tau, sep = "")), ylab = expression(paste("ac.f.(", tau, ")",
      sep = ""
   )), main = ""
)
...
acf(
   ...
   xlab = expression(paste("lag ", tau, sep = "")), ylab = expression(paste("pac.f.(", tau, ")",
      sep = ""
   )), main = ""
)
...
plot(
   ...
   main = "p-values for Ljung-Box statistic", col = "green4", font.main = 1, cex.main = 1
)
lines(x = c(-5, max.lag + 5), y = 0.05 * c(1, 1), col = "red", lty = 2)
mtext(
   text = paste("Exercise 6.3 (a): ", ts.desc, " (raw series) - diagnostic plots", sep = ""),
