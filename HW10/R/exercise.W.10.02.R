#
# R code for problem 2 of homework 10
#
...
step.dummy <- as.matrix(german.data[, "step"])
colnames(step.dummy) <- "step.dummy"
...
# plot time series
pdf("HW10/figures/exercise.W.10.02 - German macro time series.pdf")
...
plot(
   x = diff(mts.data[, mts.var.names[2]], 1), type = "b",
   xlab = "time", ylab = mts.units[2], col = "green4"
)
title(main = paste("change in", mts.desc[2]), font.main = 1, cex.main = 1)
mtext(
   text = paste("Exercise 10.2: German macro time series"), side = 3,
...
exogen.var.list <- list(NULL, step.dummy)
exogen.desc.list <- list("without dummy variable", "with step-dummy variable")

for (r in 1:2) {
   # search for best VAR model
   mts.data.VAR.ic.list[[r]] <- VARselect(
      y = mts.data, lag.max = 4 * 4,
      type = "both", season = NULL, exogen = exogen.var.list[[r]]
   )
   mts.data.VAR.fit.list[[r]] <- VAR(
      y = mts.data, type = "both",
      season = NULL, exogen = exogen.var.list[[r]],
      lag.max = 4 * 4, ic = VAR.ic
   )

   # open connection to write output to file
   output.file <- paste("HW10/figures/exercise.W.10.02 - VAR model - ",
   ...
   cat("Portmanteau test:\n")
   print(serial.test(
      x = mts.data.VAR.fit.list[[r]],
      lags.pt = 6 * 4, type = "PT.adjusted"
   ))

   # close connection to write output to file and display file
   sink()
   file.show(output.file, title = paste("exercise.W.10.02 - VAR model - ",
      exogen.desc.list[[r]], " - selection and estimation", ".txt",
      sep = ""
   ))
   ...
   for (s in 1:2) {
      pdf(paste(
         "HW10/figures/exercise.W.10.02 - VAR model - ",
      ...
      mtext(text = paste("Exercise 10.2: VAR model - ",
         exogen.desc.list[[r]], " - diagnostic plots\n(equation for ",
         mts.desc[s], ")",
         sep = ""
      ), side = 3, line = 6, outer = T, font = 2, cex = 1)
      mtext(paste("model selection criterion: ", VAR.ic, "\nbest model: VAR(",
         mts.data.VAR.fit.list[[r]]$p, ")",
         sep = ""
